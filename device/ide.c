...
// 1. 作为扫描分区表的标记   2. 记录总扩展分区地址
struct  list partition_list;                    // 分区表扫描函数
...
} __attribute__((packed));                      // 保证此结构是16字节大
// 不允许gcc编译器往里面再添加东西

// 引导扇区,mbr或ebr所在的扇区
struct boot_sector
{
    // 引导代码，这个是没用的，让partition_table[4]位置正确
    uint8_t other[446];
...
{
    // 利用device寄存器中的dev位，该位为0表示是通道中的主盘，1表示通道的从盘
...
static void select_sector(struct disk *hd, uint32_t lba, uint8_t sec_cnt)
{
    ...
    // 因为lba地址的24~27位要存储在device寄存器的0～3位,无法单独写入这4位,所以在此处把device寄存器再重新写入一次
