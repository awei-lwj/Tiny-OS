...
struct ioqueue kbd_buf;             // 定义键盘缓冲

// 定义以下变量记录相应键是否按下的状态,ext_scancode用于记录makecode是否以0xe0开头
static bool ctrl_status, shift_status, alt_status, caps_lock_status, ext_scancode;

// 以通码make_code为索引的二维数组,定义了与shift组合时的字符效果。数组范围是0~0x3A
static char keymap[][2] = 
{
// 扫描码未与shift组合
//---------------------------------------------------------------------------------------------------
    /* 0x00 */ {0, 0},     /* 没有通码为0的健，因此数组第0个一维数组keymap[0][]，将其定义为两个0*/
    /* 0x01 */ {esc, esc}, /* 第0个元素->某个按键未与shift键组合时对应的字符ASCII码值         */
    /* 0x02 */ {'1', '!'}, /* 第1个元素是某个按键与shift键组合时对应的字符ASCII码值。         */
    /* 0x03 */ {'2', '@'},
//---------------------------------------------------------------------------------------------------
...
static void intr_keyboard_handler(void)
{
    ...
    bool ctrl_down_last = ctrl_status;          // ctrl
    bool shift_down_last = shift_status;        // shift
    bool caps_lock_last = caps_lock_status;     // caps lock

    bool break_code;
    uint16_t scancode = inb(KBD_BUF_PORT);      // 从KBD_BUF_PORT获得扫描键
    ...
    if (break_code) // 若是断码break_code(按键弹起时产生的扫描码)
    {
        ...
        {
            alt_status = false; // 由于caps_lock不是弹起后关闭,所以需要单独处理
        }
    ...
    else if ((scancode > 0x00 && scancode < 0x3b) || (scancode == alt_r_make) || (scancode == ctrl_r_make))
    {
        ...
        // 判断是否与shift组合,用来在一维数组中索引对应的字符，也就是双字符
        bool shift = false;
        ...
        } // end if(判断是否与shift组合,用来在一维数组中索引对应的字符)
        ...
        if (cur_char)
        {
             ...
             * cur_char的asc码-字符a的asc码, 此差值=比较小,
            ...
            // 若kbd_buf中未满并且待加入的cur_char不为0,则将其加入到缓冲区kbd_buf
            if (!ioq_full(&kbd_buf))
            {
        ...
        } //end if 记录本次是否按下了下面几类控制键之一,供下次键入时判断组合键
    }
    else
    {
