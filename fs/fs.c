...
{
    char *part_name = (char *)arg;        // 将arg还原成字符指针
...
static void partition_format(struct partition *part)
{
    ...
    //--------------------------------------------------------------------------------------------------------------------
    // 一个块大小是一扇区
    uint32_t boot_sector_sects  = 1;     // boot sector引导块占用的扇区
    uint32_t super_block_sects  = 1;     // super block超级块占用的扇区赋值
    ...
    struct disk *hd = part->my_disk;                    // 获取分区part自己所属的硬盘
    ...
    i->i_size       = sb.dir_entry_size * 2;     // ./ 和 ../赋值为两个目录项的大小
    i->i_no         = 0;                         // 根目录占inode数组中第0个inode
    i->i_sectors[0] = sb.data_start_lba;         // 由于上面的memset,i_sectors数组的其它元素都初始化为0

    ide_write(hd, sb.inode_table_lba, buf, sb.inode_table_sects); // 最后写入硬盘

    /* 5 将根目录初始化并写入sb.data_start_lba */
    // 写入根目录的两个目录项.和..
    memset(buf, 0, buf_size);
    struct dir_entry *p_de = (struct dir_entry *)buf; // 把buf转换为目录项struct dir_entry* p_de指针
...
int32_t sys_open(const char *pathname, uint8_t flags)
{
    ...
    ASSERT(flags <= 7);        // 限制flags的值在O_RDONLY | O_WRONLY | O_RDWR | O_CREAT 之内
    ...
    switch (flags & O_CREAT)
    {
        ...
        default:     // 其余为打开文件.O_RDONLY,O_WRONLY,O_RDWR
...
// 在inode编号为p_inode_nr的目录中查找inode编号为c_inode_nr的子目录的名字,将名字存入缓冲区path.成功返回0,失败返-1
static int get_child_dir_name(uint32_t p_inode_nr, uint32_t c_inode_nr, char *path, void *io_buf)
{
    /**
     * @brief int get_child_dir_name(uint32_t p_inode_nr, uint32_t c_inode_nr, char* path, void* io_buf)
     * 
...
{
    /**
     * @brief char* sys_getcwd(char* buf, uint32_t size)
     * 
...
void filesys_init(void)
{
        ...
        while (dev_no < 2)
        {
            ...
            while (part_idx < 12)    // 4个主分区+8个逻辑
            {
                ...
                if (part->sec_cnt != 0)     // 如果分区存在
                {
                    ...
                    // 读出分区的超级块,根据数是否正确来判断是否存在文件系统
                    ide_read(hd, part->start_lba + 1, sb_buf, 1);

                    // 只支持自己的文件系统.若磁盘上已经有文件系统就不再格式化了
                    if (sb_buf->magic == 0x19491010)
                    {
    ...
    } // end while(channel_no < channel_cnt)
