...
void inode_sync(struct partition *part, struct inode *inode, void *io_buf)
{
    ...
    char *inode_buf = (char *)io_buf;           // 此缓冲区拼接同步inode的数据
...
struct inode *inode_open(struct partition *part, uint32_t inode_no)
{
    ...
    // 为使通过sys_malloc创建的新inode被所有任务共享,需要将inode置于内核空间,故需要临时
...
void inode_release(struct partition *part, uint32_t inode_no)
{
     ...
     * 1. inode位图
     * 2. inode_table
     * 3. inode中i_sector[0~11]中的直接块和一级间接索引表i_sector[12]中的中间块
     * 4. 一级间接索引表本身的扇区地址
     */
    ...
    // 1.B 如果一级间接块表存在,将其128个间接块读到all_blocks[12~], 并释放一级间接块表所占的扇区
